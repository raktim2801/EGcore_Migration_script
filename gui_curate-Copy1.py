...
def validate(mappingFile, cv):
    ...
    return study2, DATA, validationDict
...
def main():
    ...
    if validate:
        ...
        StudyDict["Assay Technology Type"]=SEInput.loc[SID]['Assay Technology Type']
        ...
        if all(dataValidation):
            ...
            st.error(f"DATA VALIDATION FAILED.")
            st.write(validationDict)
            editedMappingFile = st.data_editor(mappingFile, on_change=None)
        
            try:
                revalidate = st.checkbox('Re-Validate and Re-Curate')
                if revalidate:
                    study2, DATA, validationDict = validate(editedMappingFile, cv)
                    st.write(validationDict)
                    st.session_state.se=study2
                    st.session_state.data=DATA
                ...
                ''
            #st.write(editedMappingFile)
        #col1, col2 = st.columns(2)
        
        st.caption("Study_Experiment")
        st.dataframe(st.session_state.se)
        st.caption("Data")
        st.dataframe(DATA)
        
        
        study2= st.session_state.se
